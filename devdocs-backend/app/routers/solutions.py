...
# ============================================================================
# POST /api/solutions/suggest-tags  — AI-powered tag suggestions
# ============================================================================
...
@router.post("/solutions/suggest-tags")
async def suggest_tags(
    payload: SuggestTagsRequest,
    db: AsyncSession = Depends(get_db),
    current_user: CurrentUser = Depends(get_current_user),
) -> dict:
    """Return 3-6 AI-suggested tags for a solution using Gemini 2.0 Flash."""
    from app.services import gemini

    if not gemini.is_available():
        raise HTTPException(status_code=503, detail="AI tagging unavailable — GEMINI_API_KEY not configured")

    user = await get_or_create_user(current_user, db)

    # Fetch a sample of existing tags from the user's library for consistency
    existing_tags_query = text(
        "SELECT DISTINCT unnest(tags) as tag FROM solutions "
        "WHERE user_id = :uid AND is_archived = FALSE LIMIT 60"
    )
    result = await db.execute(existing_tags_query, {"uid": str(user.id)})
    existing_tags = [row.tag for row in result.fetchall()]

    tags = await gemini.suggest_tags(
        payload.title, payload.description, payload.code, existing_tags
    )
    return {"tags": tags}
